...
function reducer(state, action) {
  switch (action.type) {
    case 'SET_JOB':        return { ...state, jobId: action.jobId, status: 'processing', error: null, errorLogPath: null, results: null }
    ...
    case 'SET_ERROR':      return {
      ...
      status: 'error',
      error: action.error,
      errorLogPath: action.errorLogPath ?? null,
...
export function AnalysisProvider({ children }) {
  ...
  const setError      = useCallback((error, errorLogPath = null) => dispatch({ type: 'SET_ERROR', error, errorLogPath }), [])
